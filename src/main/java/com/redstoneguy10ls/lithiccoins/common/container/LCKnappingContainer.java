...
public class LCKnappingContainer extends ItemStackContainer implements ButtonHandlerContainer, ISlotCallback {

    public static final int SLOT_OUTPUT = 0;

    public static LCKnappingContainer create(ItemStack stack, KnappingType type, InteractionHand hand, int slot, Inventory playerInventory, int windowId)
    {
        return new LCKnappingContainer(LCContainerTypes.WAX_KNAPPING.get(),type,windowId,playerInventory,stack,hand,slot).init(playerInventory, 20);
    }
    private final KnappingType knappingType;

    private final LCKnappingPattern pattern;
    private final ItemStack originalStack;
    private final LCKnappingContainer.Querys querys;

    private boolean requiresReset;
    private boolean hasBeenModified;
    private boolean hasConsumedIngredient;

    public LCKnappingContainer(MenuType<?> containerType, KnappingType knappingType, int windowId, Inventory playerInv, ItemStack stack, InteractionHand hand, int slot)
    {
    ...
    public record Querys(LCKnappingContainer container) implements EmptyInventory {}
}
